(() => {
  ...
  const els = {
    ...
    draftResultPanel: $("draftResultPanel"),
    resultGameTitle: $("resultGameTitle"),
    resultABans: $("resultABans"),
    resultBBans: $("resultBBans"),
    resultAPicks: $("resultAPicks"),
    resultBPicks: $("resultBPicks"),
  ...
  function renderDraftResult() {
    if (!els.draftResultPanel || !currentRoom) return;

    const bansA = currentRoom.bansA || [];
    const bansB = currentRoom.bansB || [];
    const picksA = currentRoom.picksA || [];
    const picksB = currentRoom.picksB || [];

    const isDraftComplete =
      currentRoom.status === "finished" ||
      (bansA.length >= 4 && bansB.length >= 4 && picksA.length >= 5 && picksB.length >= 5);

    els.draftResultPanel.hidden = !isDraftComplete;

    if (!isDraftComplete) return;

    const gameNumber = currentRoom.gameNumber || 1;
    els.resultGameTitle.textContent = `Game ${gameNumber} Draft Result`;

    renderResultBoxes(els.resultABans, bansA, 4, "ban");
    renderResultBoxes(els.resultBBans, bansB, 4, "ban");
    renderResultBoxes(els.resultAPicks, picksA, 5, "pick");
    renderResultBoxes(els.resultBPicks, picksB, 5, "pick");
  }
  ...
  function renderResultBoxes(container, heroIds, count, type) {
    ...
    for (let index = 0; index < count; index++) {
      ...
      if (!heroId) {
        box.className = `result-hero-box ${type} empty`;
        box.textContent = `${type === "ban" ? "Ban" : "Pick"} ${index + 1}`;
      ...
      const heroImage = hero && hero.image ? hero.image : "";

      box.className = `result-hero-box ${type} filled`;

      const avatar = heroImage
        ? `<span class="result-avatar"><img src="${escapeHtml(heroImage)}" alt="${escapeHtml(heroName)}" onerror="this.parentElement.textContent='${heroInitials(heroName)}'"></span>`
        : `<span class="result-avatar">${heroInitials(heroName)}</span>`;

      box.innerHTML = `
        ${avatar}
        <span class="result-hero-info">
          <span class="result-hero-name">${escapeHtml(heroName)}</span>
          <span class="result-hero-order">${type === "ban" ? "Ban" : "Pick"} ${index + 1}</span>
        </span>
      `;
